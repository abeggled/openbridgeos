...
[ -n "$(manifest_value provision_script "${MANIFEST_FILE}")" ] || fail "provision_script must not be empty"
[ -n "$(manifest_value first_boot_service "${MANIFEST_FILE}")" ] || fail "first_boot_service must not be empty"
